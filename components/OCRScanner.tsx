  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 bg-zinc-800/60 hover:bg-zinc-700/60 border border-zinc-300 dark:border-zinc-700 hover:border-zinc-600 rounded-lg text-xs font-mono text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition-all"
      >
      ...
      >
        <div className="space-y-4">
          ...
          {!items.length && !saved && (
            <div
              className="border-2 border-dashed border-zinc-300 dark:border-zinc-700 hover:border-blue-500/50 rounded-xl p-8 text-center cursor-pointer transition-colors"
            ...
            >
              ...
              {file ? (
                <div className="space-y-3">
                  ...
                  <p className="text-sm font-mono text-zinc-600 dark:text-zinc-300">{file.name}</p>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">Drop invoice image or PDF here</p>
          ...
          {items.length > 0 && !saved && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-medium text-emerald-500 dark:text-emerald-400">{items.length} item{items.length !== 1 ? 's' : ''} extracted — review and edit before saving</p>
              </div>
              {items.map((item, idx) => (
                <div key={idx} className="bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Item {idx + 1}</span>
                    <button onClick={() => removeItem(idx)} className="text-zinc-600 hover:text-red-500 dark:hover:text-red-400 transition-colors">
          ...
          {saved && (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <p className="text-sm font-semibold text-emerald-500 dark:text-emerald-400">Stock updated successfully</p>
          ...
          {error && (
            <div className="bg-red-500/5 border border-red-500/20 rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-xs font-semibold text-red-500 dark:text-red-400">Issues</p>
              </div>
              <p className="text-xs font-mono text-red-500 dark:text-red-400/80 whitespace-pre-wrap">{error}</p>
            </div>
          )}
        </div>
      </Modal>
