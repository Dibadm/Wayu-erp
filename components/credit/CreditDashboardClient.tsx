    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="glass-card p-5">
          ...
          <p className="stat-num text-2xl text-red-500 dark:text-red-400">{data.overdueCount}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-card overflow-hidden">
          ...
          <div className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
            {recentApplications.map((a: any) => (
              <div key={a.id} className="px-5 py-3 flex items-center justify-between hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                <div>
                  <p className="text-sm text-zinc-700 dark:text-zinc-200">{a.applicationNo}</p>
        ...
        <div className="glass-card overflow-hidden">
          ...
          <div className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
            {openCases.map((c: any) => (
              <div key={c.id} className="px-5 py-3 flex items-center justify-between hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                <div>
                  <p className="text-sm text-zinc-700 dark:text-zinc-200">{c.caseNo}</p>
                  <p className="text-[10px] font-mono text-zinc-600">{c.customer?.name}</p>
                </div>
                <div className="text-right">
                  <p className="stat-num text-sm text-amber-500 dark:text-amber-400">ETB {Number(c.amount).toLocaleString()}</p>
      ...
      <div className="glass-card overflow-hidden">
        ...
        <div className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
          {recentNotifications.map((n: any) => (
            <div key={n.id} className="px-5 py-3 flex items-center justify-between hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
              <div>
                <p className="text-sm text-zinc-700 dark:text-zinc-200">{n.customer?.name}</p>
                <p className="text-[10px] font-mono text-zinc-600">{n.daysOutstanding} days outstanding</p>
              </div>
              <div className="text-right">
                <p className="stat-num text-sm text-amber-500 dark:text-amber-400">ETB {Number(n.amount).toLocaleString()}</p>
