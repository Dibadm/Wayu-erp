...
export default function BankRecPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Bank Reconciliation</h1>
        <p className="text-sm text-zinc-500 font-mono mt-0.5">Reconcile book vs bank (CF19 Recon1888 / All Bank)</p>
      </div>

      <div className="glass-card p-5">
        <h2 className="text-sm font-semibold mb-3 flex items-center gap-2"><Plus className="w-4 h-4 text-blue-600 dark:text-blue-400" /> New Reconciliation</h2>
      ...
      <div className="glass-card overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-2">
          <Landmark className="w-4 h-4 text-purple-600 dark:text-purple-400" /><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">History</h2>
        </div>
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
              {recs.map(r => (
                <tr key={r.id} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                  <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{r.title}</td>
                  <td className="px-4 py-2.5 text-xs font-mono text-zinc-500">{new Date(r.asOf).toLocaleDateString()}</td>
                  <td className="px-4 py-2.5 stat-num text-sm text-zinc-500 dark:text-zinc-400">{fmt(Number(r.bookBalance))}</td>
                  <td className="px-4 py-2.5 stat-num text-sm text-zinc-500 dark:text-zinc-400">{fmt(Number(r.bankBalance))}</td>
