...
const PO_STATUS_COLORS: Record<string, string> = {
  ...
  ORDERED:            'bg-amber-500/10 text-amber-500 dark:text-amber-400 border-amber-500/20',
...
export default async function SupplierDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl">
      <Link href="/suppliers" className="inline-flex items-center gap-1 text-xs font-mono text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">
      ...
      <div className="glass-card p-6">
        ...
        {/* Contact details */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-4 border-t border-zinc-200 dark:border-zinc-800">
          {supplier.email && (
            <div>
              ...
              <p className="text-xs font-mono text-zinc-600 dark:text-zinc-300 mt-1 break-all">{supplier.email}</p>
            </div>
          )}
          {supplier.phone && (
            <div>
              ...
              <p className="text-xs font-mono text-zinc-600 dark:text-zinc-300 mt-1">{supplier.phone}</p>
            </div>
          )}
          {supplier.taxNumber && (
            <div>
              <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest">Tax Number</p>
              <p className="text-xs font-mono text-zinc-600 dark:text-zinc-300 mt-1">{supplier.taxNumber}</p>
            </div>
          )}
          <div>
            <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest">Total Spend</p>
            <p className="stat-num text-sm text-zinc-900 dark:text-zinc-100 mt-1">ETB {totalSpend.toLocaleString('en-ET', { minimumFractionDigits: 2 })}</p>
          </div>
        </div>
      ...
      <div className="glass-card overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Purchase Order History</h2>
        ...
        <div className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
          {supplier.purchaseOrders.map(po => (
            <Link key={po.id} href={`/purchase-orders/${po.id}`}
              className="flex items-center gap-4 px-5 py-3 hover:bg-zinc-100 dark:hover:bg-white/[0.02] transition-colors group">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-xs font-mono font-semibold text-zinc-700 dark:text-zinc-200">{po.poNumber}</p>
              ...
              <p className="stat-num text-sm text-zinc-600 dark:text-zinc-300 flex-shrink-0">
