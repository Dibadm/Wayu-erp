  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass-card overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-800 flex items-center gap-2"><PieChart className="w-4 h-4 text-amber-400" /><h2 className="text-sm font-semibold text-zinc-100">Budget vs Actual</h2></div>
        {loading ? <div className="p-10 flex justify-center"><RefreshCw className="w-5 h-5 animate-spin text-zinc-500" /></div> : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-zinc-800">{['Category', 'Period', 'Planned', 'Inflows', 'Outflows', 'Variance'].map(h => <th key={h} className="px-4 py-3 text-left text-[10px] font-mono text-zinc-600 uppercase tracking-widest">{h}</th>)}</tr></thead>
