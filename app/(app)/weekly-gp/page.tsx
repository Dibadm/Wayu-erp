  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Weekly Gross Profit</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="glass-card p-5"><p className="text-[10px] font-mono uppercase text-zinc-500">Total Gross</p><p className="stat-num text-xl text-emerald-500 dark:text-emerald-400">{fmt(data?.totalGross ?? 0)}</p></div>
        <div className="glass-card p-5"><p className="text-[10px] font-mono uppercase text-zinc-500">Total Profit</p><p className="stat-num text-xl text-purple-600 dark:text-purple-400">{fmt(data?.totalProfit ?? 0)}</p></div>
      </div>

      <div className="glass-card overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-2">
          <CalendarDays className="w-4 h-4 text-blue-600 dark:text-blue-400" /><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Weekly Rollup</h2>
        </div>
              ...
              {(data?.rows ?? []).map((r: any, i: number) => {
                ...
                return (
                  <tr key={i} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                    <td className="px-4 py-2.5 text-xs font-mono text-zinc-600 dark:text-zinc-300">{r.label}</td>
                    <td className="px-4 py-2.5 stat-num text-sm text-zinc-500 dark:text-zinc-400">{r.qty}</td>
                    <td className="px-4 py-2.5 stat-num text-sm text-emerald-500 dark:text-emerald-400">{fmt(r.gross)}</td>
