        ...
        <div className="space-y-4">
          {result.type === 'daily' && (
            <div className="glass-card p-5">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div><p className="text-[10px] font-mono text-zinc-600">Inflows</p><p className="stat-num text-emerald-500 dark:text-emerald-400">{fmt(result.totalInflows)}</p></div>
                <div><p className="text-[10px] font-mono text-zinc-600">Outflows</p><p className="stat-num text-red-500 dark:text-red-400">{fmt(result.totalOutflows)}</p></div>
          ...
          {result.type === 'bank-balance' && (
            <div className="glass-card overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800"><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Bank Balance Report</h2></div>
                  ...
                  <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                    {result.accounts?.map((a: any) => (
                      <tr key={a.id} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                        <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{a.accountName}</td>
          ...
          {result.type === 'expense-analysis' && (
            <div className="glass-card overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800"><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Expense Analysis</h2></div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-zinc-200 dark:border-zinc-800">
                  ...
                  <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                    {result.byCategory?.map((r: ReportRow, i: number) => (
                      <tr key={i} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                        <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{r.category.replace('_', ' ')}</td>
                        <td className="px-4 py-2.5 stat-num text-sm text-red-500 dark:text-red-400 text-right">{fmt(r.total)}</td>
                        <td className="px-4 py-2.5 text-xs font-mono text-zinc-500 text-right">{(r.pct ?? 0).toFixed(1)}%</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot><tr className="border-t border-zinc-200 dark:border-zinc-800">
                    <td className="px-4 py-2.5 text-sm font-semibold text-zinc-900 dark:text-zinc-100">Grand Total</td>
                    <td className="px-4 py-2.5 stat-num text-sm font-semibold text-red-500 dark:text-red-400 text-right">{fmt(result.grandTotal)}</td>
                    <td></td>
                  </tr></tfoot>
                </table>
              </div>
          ...
          {result.type === 'budget-vs-actual' && (
            <div className="glass-card overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800"><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Budget vs Actual</h2></div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-zinc-200 dark:border-zinc-800">
                    <th className="px-4 py-3 text-left text-[10px] font-mono text-zinc-600 uppercase">Category</th>
                  ...
                  <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                    {result.budgets?.map((b: any, i: number) => (
                      <tr key={i} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                        ...
                        <td className="px-4 py-2.5 stat-num text-sm text-emerald-500 dark:text-emerald-400 text-right">{fmt(b.actualInflows ?? 0)}</td>
                        <td className="px-4 py-2.5 stat-num text-sm text-red-500 dark:text-red-400 text-right">{fmt(b.actualOutflows ?? 0)}</td>
          ...
          {result.type === 'expense-category-summary' && (
            <div className="glass-card overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800"><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Expense Category Summary</h2></div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-zinc-200 dark:border-zinc-800">
                    <th className="px-4 py-3 text-left text-[10px] font-mono text-zinc-600 uppercase">Category</th>
                  ...
                  <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                    {result.summary?.map((s: any, i: number) => (
                      <tr key={i} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                        <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{s.category.replace('_', ' ')}</td>
                        <td className="px-4 py-2.5 stat-num text-sm text-red-500 dark:text-red-400 text-right">{fmt(s.total)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {result.type === 'loan-repayment' && result.loan && (
            <div className="glass-card p-5">
              ...
              <div className="grid grid-cols-4 gap-4 mb-4">
                ...
                <div><p className="text-[10px] font-mono text-zinc-600">Total Repaid</p><p className="stat-num text-emerald-500 dark:text-emerald-400">{fmt(result.summary.totalRepaid)}</p></div>
                <div><p className="text-[10px] font-mono text-zinc-600">Interest</p><p className="stat-num text-red-500 dark:text-red-400">{fmt(result.summary.totalInterest)}</p></div>
                  ...
                  <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                    {result.repayments.map((r: any, i: number) => (
                      <tr key={i} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                        ...
                        <td className="px-4 py-2.5 stat-num text-sm text-emerald-500 dark:text-emerald-400 text-right">{fmt(r.principal)}</td>
                        <td className="px-4 py-2.5 stat-num text-sm text-red-500 dark:text-red-400 text-right">{fmt(r.interest)}</td>
          ...
          {result.type === 'investment-report' && (
            <div className="glass-card overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800"><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Investment Report</h2></div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-zinc-200 dark:border-zinc-800">
                  ...
                  <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                    {result.investments?.map((inv: any, i: number) => (
                      <tr key={inv.id ?? i} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                        <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{inv.name}</td>
                        ...
                        <td className="px-4 py-2.5 stat-num text-sm text-emerald-500 dark:text-emerald-400 text-right">{fmt(inv.expectedReturn)}</td>
