...
interface BankTransfer { id: string; amount: number; description: string | null; reference: string | null; transferredAt: string; fromAccount: { accountName: string }; toAccount: { accountName: string }; createdBy: { name: string } }
  ...
  async function load() {
    ...
    try {
      const [t, a] = await Promise.all([
        fetch('/api/bank-transfers').then(r => r.json()).catch(() => []),
        ...
        <div className="glass-card p-5">
          ...
          <form onSubmit={add} className="grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
            <select className="input w-full" value={form.fromAccountId} onChange={e => setForm({ ...form, fromAccountId: e.target.value })} required>
              ...
              {accounts.map(a => <option key={a.id} value={a.id}>{a.accountName} (Balance: {fmt(a.currentBalance)})</option>)}
            </select>
