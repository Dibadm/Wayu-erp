...
export default function ImportExportPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Import / Export Center</h1>
      ...
      {msg && <div className="glass-card p-3 text-sm font-mono text-emerald-500 dark:text-emerald-400 flex items-center gap-2"><CheckCircle2 className="w-4 h-4" /> {msg}</div>}
      ...
      {tab === 'import' && (
        <div className="glass-card p-5">
          ...
          <p className="text-[11px] font-mono text-zinc-600 mt-3 flex items-center gap-1">
            <AlertTriangle className="w-3 h-3 text-amber-500 dark:text-amber-400" /> Keys: receiptNumber (sales) / poNumber (purchases). Mismatches are recorded, never overwritten.
          </p>
        </div>
      )}
      ...
      {tab === 'export' && (
        <div className="space-y-4">
          <div className="glass-card p-5 flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs font-mono text-zinc-500 dark:text-zinc-400">Calendar:</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {EXPORTS.map(x => (
              <div key={x.sheet} className="glass-card p-5 flex flex-col gap-3">
                <div><h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{x.title}</h3><p className="text-xs text-zinc-500 mt-0.5">{x.desc}</p></div>
      ...
      {tab === 'history' && (
        <div className="space-y-6">
          <div className="glass-card overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800"><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Import History</h2></div>
                ...
                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                  {batches.map(b => (
                    <tr key={b.id} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                      <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{b.fileName}</td>
                      <td className="px-4 py-2.5 text-xs text-zinc-500">{b.importedBy.name}</td>
                      <td className="px-4 py-2.5 stat-num text-sm text-zinc-500 dark:text-zinc-400">{b.rowCount}</td>
                      <td className="px-4 py-2.5 stat-num text-sm text-emerald-500 dark:text-emerald-400">{b.newRows}</td>
          ...
          <div className="glass-card overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-amber-500 dark:text-amber-400" /><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Unresolved Sync Conflicts</h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                  {conflicts.map(c => (
                    <tr key={c.id} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                      <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{c.entityName}</td>
                      <td className="px-4 py-2.5 text-xs font-mono text-zinc-500">{c.field}</td>
                      <td className="px-4 py-2.5 stat-num text-sm text-amber-500 dark:text-amber-400">{c.excelValue}</td>
                      <td className="px-4 py-2.5 stat-num text-sm text-zinc-500 dark:text-zinc-400">{c.appValue}</td>
                      <td className="px-4 py-2.5 text-xs font-mono text-zinc-500">{c.importBatch.fileName}</td>
                    </tr>
                  ))}
                  {conflicts.length === 0 && <tr><td colSpan={5} className="px-4 py-10 text-center text-xs font-mono text-emerald-500 dark:text-emerald-400 flex items-center justify-center gap-2"><CheckCircle2 className="w-4 h-4" /> No conflicts — app matches Excel.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
