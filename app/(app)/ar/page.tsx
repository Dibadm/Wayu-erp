...
export default function ARPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Accounts Receivable</h1>
      ...
      <div className="glass-card p-5 max-w-xs">
        ...
        <p className="stat-num text-2xl text-amber-500 dark:text-amber-400">{fmt(data?.totalOutstanding ?? 0)}</p>
      </div>

      <div className="glass-card overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-2">
          <AlertTriangle className="w-4 h-4 text-amber-500 dark:text-amber-400" />
          <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Customers with Balance</h2>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
              {(data?.ledger ?? []).map((c: any) => (
                <tr key={c.customerId} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                  <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{c.name}</td>
                  <td className="px-4 py-2.5 font-mono text-xs text-zinc-500">{c.tinNo ?? '—'}</td>
                  <td className="px-4 py-2.5 stat-num text-sm text-amber-500 dark:text-amber-400">{fmt(c.outstanding)}</td>
