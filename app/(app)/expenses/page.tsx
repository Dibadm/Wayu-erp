...
export default function ExpensesPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Expenses</h1>
        <p className="text-sm text-zinc-500 font-mono mt-0.5">General ledger — by category (CF19 Trans → All Exp)</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="glass-card p-5"><p className="text-[10px] font-mono uppercase text-zinc-500">Total Debit</p><p className="stat-num text-xl text-red-500 dark:text-red-400">{fmt(report?.totalDebit ?? 0)}</p></div>
        <div className="glass-card p-5"><p className="text-[10px] font-mono uppercase text-zinc-500">Total Credit</p><p className="stat-num text-xl text-emerald-500 dark:text-emerald-400">{fmt(report?.totalCredit ?? 0)}</p></div>
        <div className="glass-card p-5"><p className="text-[10px] font-mono uppercase text-zinc-500">Net</p><p className="stat-num text-xl text-zinc-600 dark:text-zinc-300">{fmt(report?.net ?? 0)}</p></div>
      </div>

      <div className="glass-card p-5">
        <h2 className="text-sm font-semibold mb-3 flex items-center gap-2"><Plus className="w-4 h-4 text-blue-600 dark:text-blue-400" /> Add Expense</h2>
      ...
      <div className="glass-card overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-2">
          <Receipt className="w-4 h-4 text-purple-600 dark:text-purple-400" /><h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Expense Ledger</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
              {list.map(e => (
                <tr key={e.id} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                  <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{e.category}</td>
                  ...
                  <td className="px-4 py-2.5 text-right"><button onClick={() => remove(e.id)} className="text-red-500 dark:text-red-400"><Trash2 className="w-4 h-4" /></button></td>
                </tr>
              ))}
