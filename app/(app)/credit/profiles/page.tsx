  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Credit Profiles</h1>
          <p className="text-sm text-zinc-500 font-mono mt-0.5">Customer credit limits and risk levels</p>
        </div>
        {canEdit && (
          <button onClick={() => { setShowForm(true); setEditingId(null); setForm({ customerId: '', creditLimit: '', paymentTerms: '30', notes: '' }) }} className="flex items-center gap-1.5 px-3 py-2 bg-blue-500/10 border border-blue-500/20 rounded-lg text-sm text-blue-600 dark:text-blue-400 hover:bg-blue-500/20 transition-colors">
      ...
      {showForm && (
        <form onSubmit={submit} className="glass-card p-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <select value={form.customerId} onChange={e => setForm({ ...form, customerId: e.target.value })} className="bg-transparent border border-zinc-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-700 dark:text-zinc-200" required disabled={!!editingId}>
              <option value="">Select customer</option>
              {customers.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <input type="number" placeholder="Credit Limit" value={form.creditLimit} onChange={e => setForm({ ...form, creditLimit: e.target.value })} className="bg-transparent border border-zinc-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-700 dark:text-zinc-200" required />
            <input type="number" placeholder="Payment Terms (days)" value={form.paymentTerms} onChange={e => setForm({ ...form, paymentTerms: e.target.value })} className="bg-transparent border border-zinc-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-700 dark:text-zinc-200" />
          </div>
          <textarea placeholder="Notes" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} className="w-full bg-transparent border border-zinc-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-700 dark:text-zinc-200" rows={2} />
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => { setShowForm(false); setEditingId(null) }} className="px-4 py-2 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 rounded-lg text-sm hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">Cancel</button>
          </div>
        </form>
      )}
      ...
      <div className="glass-card overflow-hidden">
              ...
              {profiles.map((p: any) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-zinc-100 dark:hover:bg-white/[0.02]">
                    <td className="px-4 py-2.5 text-sm text-zinc-600 dark:text-zinc-300">{p.customer?.name ?? '—'}</td>
                    <td className="px-4 py-2.5 stat-num text-sm">{fmt(p.creditLimit)}</td>
                    <td className="px-4 py-2.5 stat-num text-sm text-emerald-500 dark:text-emerald-400">{fmt(p.availableCredit)}</td>
                    <td className="px-4 py-2.5 stat-num text-sm text-amber-500 dark:text-amber-400">{fmt(p.utilizedCredit)}</td>
                    ...
                    <td className="px-4 py-2.5 flex gap-2">
                      {canEdit && <button onClick={() => edit(p)} className="p-1.5 bg-blue-500/10 border border-blue-500/20 rounded text-blue-600 dark:text-blue-400 hover:bg-blue-500/20 transition-colors"><Edit2 className="w-4 h-4" /></button>}
                      {canEdit && <button onClick={() => toggleBlock(p)} className={`p-1.5 rounded border transition-colors ${p.isBlocked ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500 dark:text-emerald-400 hover:bg-emerald-500/20' : 'bg-red-500/10 border-red-500/20 text-red-500 dark:text-red-400 hover:bg-red-500/20'}`}><Ban className="w-4 h-4" /></button>}
                    </td>
                  </tr>
                )
              })}
        ...
        <div className="px-4 py-3 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="px-3 py-1 text-xs rounded border border-zinc-300 dark:border-zinc-700 disabled:opacity-50 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">Next</button>
          </div>
        </div>
      </div>
    </div>
